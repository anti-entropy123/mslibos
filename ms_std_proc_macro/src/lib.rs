...
fn derive_verify_impl(t: proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    ...
    quote::quote! {
        impl ms_hostcall::Verify for #struct_name {
...
fn faas_arg_impl_test() {
    assert_eq!(
        ...
        "impl ms_hostcall :: Verify for my_args { fn __fingerprint () -> u64 { 13418197873283780248u64 } }"
    );

    assert_eq!(
        derive_verify_impl("struct my_args {a: String, b:i32,}".parse().unwrap()).to_string(),
        "impl ms_hostcall :: Verify for my_args { fn __fingerprint () -> u64 { 7169761655847290832u64 } }"
    );
}
