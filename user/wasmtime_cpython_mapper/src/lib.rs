...
fn func_body(my_id: &str, pyfile_path: &str, reducer_num: u64) -> Result<()> {
    let my_map_id = &format!("mapper_{}", my_id);
    ...
    #[cfg(feature = "log")]
    println!("rust: wasmtime_cpython_mapper_{:?} finished!", my_id);
...
pub fn main() -> Result<()> {
    ...
    func_body(my_id, pyfile_path, reducer_num)
}
