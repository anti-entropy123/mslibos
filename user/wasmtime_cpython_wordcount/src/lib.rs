...
fn func_body(my_id: &str, pyfile_path: &str, reducer_num: u64, mapper_num: u64) -> Result<()> {
    let my_map_id = &format!("wordcount_{}", my_id);
...
pub fn main() -> Result<()> {
    ...
    func_body(my_id, pyfile_path, reducer_num, mapper_num)
}
